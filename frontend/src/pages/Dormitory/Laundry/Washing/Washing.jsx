...
function Washing() {
  ...
  const { queue, loading, addBookingAPI } = useLaundry();
      ...
      <div className="main-content-wrapper">
        <MainContent>
          ...
          <div className="washing-container">
            ...
            <input type="date" className="washing-input" value={date} onChange={e => { setDate(e.target.value); setConfirmed(false); setError(""); }} />
            {date && <>
              <h2 className="washing-title">Выбор времени:</h2>
              <input type="time" className="washing-input" value={time} onChange={e => { setTime(e.target.value); setConfirmed(false); setError(""); }} />
            </>}
            {date && time && <>
              <h2 className="washing-title">Номер машинки:</h2>
              <input type="number" className="washing-input" placeholder="Введите номер машинки" value={machine} onChange={e => { setMachine(e.target.value); setConfirmed(false); setError(""); }} />
            </>}
            {date && time && machine && <Button text="Забронировать" onClick={handleBooking} />}
            {confirmed && <p className="washing-confirmation">✅ Машинка №{machine} успешно забронирована на {date} в {time}.</p>}
            {error && <p className="washing-error">⚠️ {error}</p>}
            {machine && date && <div className="queue-container">
              <h3 className="queue-title">Очередь на машинку №{machine}:</h3>
